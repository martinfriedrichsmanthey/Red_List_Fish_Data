...
print(specs[i])   
mod_factor_year <- brm(n_individuals ~ -1 + factor_year + year_day + offset(log(effort_m)) + (1|unique_ID), data = tmp_spec, family = negbinomial(),chains = 4)  
saveRDS(mod_factor_year, file = factor_output)
rm(mod_factor_year)
    
#### model for overall trend
print(specs[i])   
mod_index_year <- brm(n_individuals ~ index_year + year_day + offset(log(effort_m)) +(1+scaled_year|unique_ID), data = tmp_spec, family = negbinomial(),chains = 4)
